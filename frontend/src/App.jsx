...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BarChart3, Check, ChevronDown, ChevronUp, CirclePlay, Clock3, Copy, Download, Eye, EyeOff, FilePlus2, Folder, FolderOpen, GraduationCap, History, LogOut, Music2, Pause, Play, Plus, Radio, Save, Search, Share2, ShieldCheck, Trash2, UserPlus, Users, Volume2, VolumeX, X, Zap } from "lucide-react";
  ...
  return <div className="auth-page">
    ...
    <section className="auth-panel">
      <form className="auth-card" onSubmit={submit}>
        ...
        <button className="text-button" type="button" onClick={() => { setRegister(!register); setShowPassword(false); setError(""); }}>{register ? "Already have an account? Sign in" : "New instructor? Create an account"}</button>
      </form>
    </section>
  </div>;
}
...
function QuizEditor() {
  ...
  async function save() {
    ...
    setSaving(true);
    const body = { title: quiz.title, course_tag: quiz.course_tag, folder_id: quiz.folder_id ? Number(quiz.folder_id) : null, questions: quiz.questions.map(({ id, position, ...q }) => q) };
...
function Host() {
  ...
  return <div className="host-page">
    ...
    {session.status === "live" && <main className="live-host"><div className="question-stage"><div className="question-topline"><span>Question {(session.current_question_index ?? 0) + 1} / {session.question_count}</span><span><Users />{state.response_count || 0} / {participants.length} answered</span></div><h1>{question?.text}</h1><div className="host-options">{question?.options.map((option, index) => { const revealed = Boolean(state.reveal); const count = revealed ? state.distribution?.[index] || 0 : 0; const max = Math.max(1, ...(revealed ? state.distribution || [] : [])); return <div className={`host-option ${palette[index]} ${revealed ? "revealed" : "private"}`} key={index}><span className="option-key">{String.fromCharCode(65 + index)}</span><strong>{option}</strong>{revealed ? <><div className="bar-track"><i style={{ width: `${count / max * 100}%` }} /></div><b>{count}</b>{state.reveal?.correct_options?.includes(index) && <span className="correct-badge"><Check /></span>}</> : <span className="answer-hidden">Hidden</span>}</div>; })}</div></div>
...
function Player() {
  ...
  const [state, setState] = useState(null); const [selected, setSelected] = useState([]); const [submitted, setSubmitted] = useState(false); const [expired, setExpired] = useState(false); const [notice, setNotice] = useState(""); const socket = useRef(null);
  const onMessage = useCallback((message, ws) => { socket.current = ws; setState(prev => {
    if (message.type === "snapshot") { setSubmitted(Boolean(message.me?.answered)); setSelected(message.me?.selected_options || []); setExpired(Boolean(message.session?.is_revealed) || (message.question?.deadline ? Date.now() >= new Date(message.question.deadline).getTime() : false)); return message; }
    if (!prev) return prev;
    if (message.type === "question_start") { setSelected([]); setSubmitted(false); setExpired(false); setNotice(""); return { ...prev, session: { ...prev.session, status: "live", is_revealed: false, current_question_index: message.question.position, question_count: message.question_count }, question: message.question, reveal: null, leaderboard: null, me: { ...prev.me, answered: false, selected_options: [], current_result: null } }; }
  ...
  const wasCorrect = me.current_result?.is_correct ?? (selected.length > 0 && reveal && selected.every(i => reveal.correct_options.includes(i)) && selected.length === reveal.correct_options.length);
  function choose(index) { if (submitted || reveal || expired) return; if (question.type === "single") setSelected([index]); else setSelected(s => s.includes(index) ? s.filter(i => i !== index) : [...s, index]); }
  function submit() { if (!selected.length || expired || !socket.current || socket.current.readyState !== WebSocket.OPEN) return; socket.current.send(JSON.stringify({ type: "answer_submitted", question_id: question.id, selected_options: selected })); }
  ...
  return <div className="player-page play-surface"><header className="player-bar"><span>Q{(session.current_question_index ?? 0) + 1}/{session.question_count}</span><strong>{session.quiz_title}</strong><span>{(me.score || 0).toLocaleString()} pts</span></header>
    <main className="player-main">{question && <><div className="player-question"><Countdown deadline={question.deadline} onExpire={() => { setExpired(true); setNotice("Time is up — waiting for results"); }} /><h1>{question.text}</h1><p>{question.type === "multi" ? "Select all correct answers" : "Choose one answer"}</p></div><div className="player-options">{question.options.map((option, index) => { const isSelected = selected.includes(index); const isCorrect = reveal?.correct_options?.includes(index); const isWrong = reveal && isSelected && !isCorrect; return <button disabled={submitted || expired || Boolean(reveal)} onClick={() => choose(index)} className={`${palette[index]} ${isSelected ? "selected" : ""} ${isCorrect ? "revealed-correct" : ""} ${isWrong ? "revealed-wrong" : ""}`} key={index}><span>{String.fromCharCode(65 + index)}</span><strong>{option}</strong>{(isSelected || isCorrect) && <i>{isWrong ? <X /> : <Check />}</i>}</button>; })}</div>{!reveal && <div className="submit-zone"><Button onClick={submit} disabled={!selected.length || submitted || expired}>{submitted ? <><Check />Answer locked</> : expired ? "Time is up" : question.type === "multi" ? "Lock answers" : "Lock answer"}</Button>{notice && <span>{notice}</span>}</div>}</>}
